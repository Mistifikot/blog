...
export class SpineConnector {
  ...
  private spinePath: string | null = null;
   ...
   */
  private findSpineExecutable(): string | null {
    try {
   ...
   */
  private createExampleProject(): void {
    try {
   ...
   */
  getProjectDetails(projectName: string): string | null {
    try {
      ...
      try {
        ...
        if (projectData) {
          ...
          if (projectData.bones) {
            ...
            const rootBones = projectData.bones.filter((bone: any) => !bone.parent).map((bone: any) => bone.name);
          ...
          if (projectData.skins) {
            ...
            const skinNames = projectData.skins.map((skin: any) => skin.name);
   ...
   */
  exportAnimation(projectName: string, format: string = 'json', outputName?: string): string | null {
    try {
...
bone1
   ...
   */
  generateAnimation(name: string, description: string = ''): string | null {
    try {
   ...
   * @returns Путь к созданному файлу или null в случае ошибки
   */
  private generateAnimationFile(filePath: string, name: string, description: string): string | null {
    try {
   ...
   */
  openInSpine(filePath: string | null): boolean {
    try {
